                            ...
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <div class="p-4 md:p-5 space-y-4">
                                    <div>
                                        ...
                                        <input @input="vreset('name')" v-model="artist.name" type="text"
                                        ...
                                        <span class="mt-2 text-sm text-red-600 dark:text-red-500"
                                            v-if="error.errors.name && error.errors.name[0] != ''">{{
                                        error.errors.name[0]
                                    }}</span>
                                    </div>
                                    <div>
                                        <label for="name"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">First Release Year</label>
                                        <input @input="vreset('first_release_year')" v-model="artist.first_release_year" type="text"
                                            name="firstname" id="name"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                            placeholder="" required>
                                        <span class="mt-2 text-sm text-red-600 dark:text-red-500"
                                            v-if="error.errors.first_release_year && error.errors.first_release_year[0] != ''">{{
                                        error.errors.first_release_year[0]
                                    }}</span>
                                    </div>
                                    <div>
                                        <label for="name"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">No of Album</label>
                                        <input @input="vreset('no_of_album_release')" v-model="artist.no_of_album_release" type="text"
                                            name="lastname" id="lastname"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                            placeholder="" required>
                                        <span class="mt-2 text-sm text-red-600 dark:text-red-500"
                                            v-if="error.errors.no_of_album_release && error.errors.no_of_album_release[0] != ''">{{
                                        error.errors.no_of_album_release[0]
                                    }}</span>
                                    </div>
                                    <div>
                                        <label for="dob"
                                            ...
                                            Date of birth</label>
                                        <input @input="vreset('dob')" v-model="artist.dob" type="date" name="dob" id="dob"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                            placeholder="" required>
                                        <span class="mt-2 text-sm text-red-600 dark:text-red-500"
                                    ...
                                    <div>
                                        ...
                                        <input @input="vreset('address')" v-model="artist.address" type="text"
                                    ...
                                    <div>
                                        <label for="gender"
                                            ...
                                            an option</label>
                                        <select @change="vreset('gender')" id="gender" v-model="artist.gender"
                                ...
                                <div
                                    ...
                                    <button v-if="artistId == 0" type="button" @click="submitArtist"
                                    ...
                                    <button v-else type="button" @click="updateArtist"
                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg ">
                        <div
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="table-search-artist"
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 ">
                            ...
                            <tbody>
                                <tr v-for="artist in artists.data"
                                    ...
                                    <td class="px-6 py-4">
                                        <a href="#"
                                            ...
                                            @click.prevent="gotoMusic(artist.id)">View Music</a>
                                        <a href="#"
                                            ...
                                            @click="editartist(artist)">Edit</a>
                                        <a href="#"
                                            ...
                                            @click="deleteartist(artist.id)">Delete</a>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                        <Paginate :currentpage="artists.currentpage" :perpage="artists.perpage" :total="artists.total"
                            :search="''" @loadDate="getartist" />
...
const submitArtist = () => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios.setHeader()
            axios.post('api/artist-create', artist.value).then((response) => {
...
const updateArtist = () => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios.setHeader()
    axios.put('api/artist-update/' + artistId.value, artist.value).then((response) => {
        if (response.status == 200) {
            const Toast = Swal.mixin({
                toast: true,
                position: "top-end",
...
const deleteartist = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios.setHeader()
            axios.delete('api/artist-delete/' + id).then((response) => {
                if (response.status == 200) {
                    const Toast = Swal.mixin({
                        toast: true,
                        position: "top-end",
